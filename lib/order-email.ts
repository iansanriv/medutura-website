...
export async function sendOrderConfirmation(orderId: string) {
  ...
  const html = `<!doctype html><html><body style="margin:0;background:#f3fbfb;font-family:Arial,sans-serif;color:#111827"><div style="max-width:620px;margin:0 auto;padding:32px 16px"><div style="background:#fff;border-radius:24px;padding:32px"><p style="margin:0;color:#168a91;font-weight:800;letter-spacing:.12em">MEDUTURA</p><h1 style="margin:12px 0 8px;font-size:30px">${paid ? "¡Pago confirmado!" : "Recibimos tu orden"}</h1><p style="color:#475569;line-height:1.6">Hola ${escapeHtml(order.customer_name)}, tu orden <strong>${escapeHtml(order.order_number)}</strong> quedó registrada.</p>${athInstructions}<table style="width:100%;border-collapse:collapse;margin-top:20px">${itemRows}<tr><td style="padding:12px 0;border-top:1px solid #e2e8f0;font-weight:800">Total</td><td style="padding:12px 0;border-top:1px solid #e2e8f0;text-align:right;font-size:20px;font-weight:900">${money(order.total_cents)}</td></tr></table><p style="margin-top:24px;color:#64748b;line-height:1.6">${order.delivery_method === "pickup" ? "Coordinaremos contigo el recogido." : `Entrega: ${escapeHtml(order.address || "Dirección por confirmar")}.`}</p><p style="margin-top:28px;color:#0f6f74;font-weight:700">Gracias por apoyar lo hecho a mano en Puerto Rico.</p></div></div></body></html>`;
